...
typedef enum Mode { ModeNone, ModeApp, ModeWin } Mode;
...
static void InitGraphicsResources(SGraphicsResources *pRes,
  ...
  // Text
  {
    ...
    ASSERT(Ok ==
           GdipSetStringFormatAlign(pRes->_pFormat, StringAlignmentCenter));
    ASSERT(Ok ==
           GdipSetStringFormatLineAlign(pRes->_pFormat, StringAlignmentCenter));
    ASSERT(Ok ==
           GdipSetStringFormatFlags(pRes->_pFormat,
  ...
  // Colors
  {
    ...
    if (config->_ThemeMode == ThemeModeAuto) {
      ...
      DWORD valueCount = 0;
      RegQueryInfoKey(key, NULL, NULL, NULL, NULL, NULL, NULL, &valueCount,
                      NULL, NULL, NULL, NULL);
      ...
      for (uint32_t i = 0; i < valueCount; i++) {
        ...
        RegEnumValue(key, i, name, &nameSize, NULL, NULL, (BYTE *)&value,
                     &valueSize);
  ...
  // Brushes
  {
    ASSERT(Ok == GdipCreateSolidFill(pRes->_BackgroundColor | 0xFF000000,
                                     &pRes->_pBrushBg));
    ASSERT(Ok ==
           GdipCreateSolidFill(pRes->_HighlightBackgroundColor | 0xFF000000,
                               &pRes->_pBrushBgHighlight));
    ASSERT(Ok == GdipCreateSolidFill(pRes->_TextColor | 0xFF000000,
                                     &pRes->_pBrushText));
  }
}
...
static const char *WindowsClassNamesToSkip[] = {"Shell_TrayWnd",
                                                "DV2ControlHost",
                                                "MsgrIMEWindowClass",
                                                "SysShadow",
                                                "Button",
...
bool BelongsToCurrentDesktop(HWND window) {
  ...
  (void)vdm;
  CoInitialize(NULL);
  CoCreateInstance(&CLSID_VirtualDesktopManager, NULL, CLSCTX_ALL,
                   &IID_IVirtualDesktopManager, (void **)&vdm);

  WINBOOL isCurrent = true;
  if (vdm)
    IVirtualDesktopManager_IsWindowOnCurrentVirtualDesktop(vdm, window,
                                                           &isCurrent);
...
static bool IsWindowOnMonitor(HWND hwnd, HMONITOR targetMonitor) {
  ...
  // For minimized windows, use the restored position instead of current
  // position
  if (IsIconic(hwnd)) {
    WINDOWPLACEMENT wp;
  ...
  // Use CompareObjectHandles if available (Windows 10+) for more robust
  // comparison Fall back to direct comparison for older Windows versions
  ...
  if (!initialized) {
    ...
    if (kernel32) {
      pCompareObjectHandles = (BOOL(WINAPI *)(HANDLE, HANDLE))GetProcAddress(
...
static bool IsAltTabWindow(HWND hwnd) {
  ...
  for (uint32_t i = 0;
       i < sizeof(WindowsClassNamesToSkip) / sizeof(WindowsClassNamesToSkip[0]);
       i++) {
  ...
  if (!strcmp(buf, "ApplicationFrameWindow"))
    DwmGetWindowAttribute(hwnd, (DWORD)DWMWA_CLOAKED, (PVOID)&cloaked,
                          (DWORD)sizeof(cloaked));
  if (cloaked)
    return false;
  WINDOWINFO wi;
...
static void LoadIndirectString(const wchar_t *packagePath,
                               const wchar_t *packageName,
                               const wchar_t *resource, wchar_t *output) {
...
static void GetUWPIconAndAppName(HANDLE process, wchar_t *outIconPath,
                                 wchar_t *outAppName, SAppData *appData) {
  ...
  wchar_t *displayName = NULL;
  {
    ...
    HRESULT res =
        SHCreateStreamOnFileEx(manifestPath, STGM_READ | STGM_SHARE_EXCLUSIVE,
    ...
    // Appxfactory:
    // CLSID_AppxFactory and IID_IAppxFactory are declared as extern in
    // "AppxPackaging.h" I don't know where the symbols are defined, thus the
    // hardcoded GUIDs here.
    GUID clsid;
    CLSIDFromString(L"{5842a140-ff9f-4166-8f5c-62f5b7b0c781}", &clsid);
    GUID iid;
    IIDFromString(L"{beb94909-e451-438b-b5a7-d79e767b75d8}", &iid);
    res = CoCreateInstance(&clsid, NULL, CLSCTX_INPROC_SERVER, &iid,
                           (void **)&appxfac);
    if (!SUCCEEDED(res))
      return;
    ...
    res = IAppxFactory_CreateManifestReader(appxfac, inputStream,
    ...
    while (hasApp) {
      ...
      if (!wcscmp(aumid, userModelID)) {
        IAppxManifestApplication_GetStringValue(app, L"Square44x44Logo",
                                                &logoProp);
        IAppxManifestApplication_GetStringValue(app, L"DisplayName",
                                                &displayName);
        break;
      }
...

  {
    if (wcsstr(displayName, L"ms-resource:") == displayName) {
      LoadIndirectString(packagePath, pid[0].name, &displayName[12],
                         outAppName);
    } else
...
static BOOL GetIconGroupName(HMODULE hModule, LPCSTR lpType, LPSTR lpName,
                             LONG_PTR lParam) {
  (void)hModule;
  (void)lpType;
  (void)lpName;
...
static void GetAppName(const wchar_t *exePath, wchar_t *out) {
  ...
  {
    // DWORD res = CoCreateInstance(&CLSID_ShellItem, NULL,
    // CLSCTX_INPROC_SERVER, &IID_IShellItem2, (void**)&IShellItem2);
    // ASSERT(SUCCEEDED(res))
  }
  DWORD res = SHCreateItemFromParsingName(exePath, NULL, &IID_IShellItem2,
                                          (void **)&shellItem);
  ASSERT(SUCCEEDED(res))
  wchar_t *siStr = NULL;
...
static GpBitmap *GetIconFromExe(const char *exePath) {
  ...
  uint32_t resByteSize = 0;
  {
    char name[256];
    char *pName = name;
    EnumResourceNames(module, RT_GROUP_ICON, GetIconGroupName,
                      (LONG_PTR)&pName);
  ...
  {
    HRSRC iconResInfo =
        FindResource(module, MAKEINTRESOURCE(iconResID), RT_ICON);
    HGLOBAL iconRes = LoadResource(module, iconResInfo);
    BYTE *data = (BYTE *)LockResource(iconRes);
    HICON icon =
        CreateIconFromResourceEx(data, resByteSize, true, 0x00030000, 0, 0, 0);
  ...
  GpBitmap *out;
  {
    ...
    GdipCreateBitmapFromScan0(iconSize, iconSize, 4 * iconSize,
                              PixelFormat32bppARGB, NULL, &out);
    ...
    if (noAlpha && hbmMask != NULL && iconSize <= 256) {
      ...
      for (int i = 0; i < iconSize * iconSize; i++) {
        unsigned int aFromMask =
            (0x1 & (maskData[i / 8] >> (7 - i % 8))) ? 0 : 0xFF000000;
  ...
  if (group == NULL) {
    ...
    if (group->_IconBitmap == NULL) {
      ...
      GpBitmap *out;
      {
        ...
        MEM_INIT(dstData);
        GdipBitmapLockBits(out, &r, 0, PixelFormat32bppARGB, &dstData);
        GetBitmapBits(hbm, sizeof(uint32_t) * iconSize * iconSize,
                      dstData.Scan0);
        GdipBitmapUnlockBits(out, &dstData);
      }
...
static void ComputeMetrics(uint32_t iconCount, float scale, Metrics *metrics,
  ...
  const int screenHeight = monitorSize[1];
  const int screenWidth = monitorSize[0];
  const float containerRatio = 1.25f;
  float iconSize = GetSystemMetrics(SM_CXICON) * scale;
  const float padRatio = max(0.25 * iconSize, 16.0f) / iconSize;

  // Calculate dimensions for vertical list layout
  const uint32_t itemHeight = iconSize * containerRatio;
  const uint32_t maxSizeY = min(
      iconCount * itemHeight + 2.0f * padRatio * iconSize, screenHeight * 0.8);
  const uint32_t sizeY = maxSizeY;
  const uint32_t sizeX =
      min(650.0f * scale,
          screenWidth * 0.6f); // Made wider: increased from 350 to 500, and
                               // from 0.4f to 0.6f of screen width
  ...
  metrics->_Container = itemHeight;
  metrics->_Selection = itemHeight;
...
static void CreateWin(SAppData *appData) {
  ...
  ComputeMetrics(
      appData->_WinGroups._Size, appData->_Config._Scale, &appData->_Metrics,
      appData->_Config._MultipleMonitorMode == MultipleMonitorModeMouse);

  HWND hwnd =
      CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW |
                         WS_EX_LAYERED,    // Optional window styles (WS_EX_)
                     CLASS_NAME,           // Window class
                     "",                   // Window text
                     WS_BORDER | WS_POPUP, // Window style
                     // Pos and size
                     appData->_Metrics._WinPosX, appData->_Metrics._WinPosY,
                     appData->_Metrics._WinX, appData->_Metrics._WinY,
                     NULL,               // Parent window
                     NULL,               // Menu
  ...
  RECT r = {appData->_Metrics._WinPosX, appData->_Metrics._WinPosY,
  ...
  // Values are from cpp enums DWMWINDOWATTRIBUTE and
...
static void InitializeSwitchApp(SAppData *appData) {
  ...
  if (appData->_Config._AppFilterMode == AppFilterModeMouseMonitor) {
    ...
    appData->_MouseMonitor =
        NULL; // Explicitly set NULL when not filtering by monitor
  }
...
static void ApplySwitchApp(const SWinGroup *winGroup) {
  // Set focus for all win, not only the last one. This way when the active
  // window is closed, the second to last window of the group becomes the active
  // one.
  ...
  for (int i = winCount - 1; i >= 0; i--) {
    ...
    dwp = DeferWindowPos(dwp, win, prev, 0, 0, 0, 0,
                         SWP_NOSIZE | SWP_NOMOVE | SWP_NOOWNERZORDER);
    // ASSERT(dwp != 0);
    prev = win;
  }
...
static DWORD WorkerThread(LPVOID data) {
  ...
  HANDLE window =
      CreateWindowEx(WS_EX_TOPMOST, WORKER_CLASS_NAME, NULL, WS_POPUP, 0, 0, 0,
                     0, HWND_MESSAGE, NULL, appData->_Instance, appData);
  (void)window;
...
static void ApplySwitchWin(HWND win) {
  ...
  DWORD res = CoCreateInstance(&CLSID_CUIAutomation, NULL, CLSCTX_INPROC_SERVER,
...
static LRESULT KbProc(int nCode, WPARAM wParam, LPARAM lParam) {
  ...
  const bool isWatchedKey = isAppHold || isAppSwitch || isWinHold ||
                            isWinSwitch || isInvert || isTab || isShift ||
  ...
  const Mode prevMode = mode;
  {
    ...
    const bool winHoldReleasing =
        prevKeyState._HoldWinDown && !keyState._HoldWinDown;
    const bool appHoldReleasing =
        prevKeyState._HoldAppDown && !keyState._HoldAppDown;
    const bool escapeInput = isEscape && !rel;

    const bool switchApp = switchAppInput && keyState._HoldAppDown;
    const bool prevApp = prevAppInput && keyState._HoldAppDown;
    const bool switchWin = switchWinInput && keyState._HoldWinDown;
    const bool cancel = escapeInput && keyState._HoldAppDown;

    // Denit.
    if ((prevMode == ModeApp) && (switchWin || appHoldReleasing) && !prevApp) {
      mode = ModeNone;
      bypassMsg = true;
      PostThreadMessage(_MainThread, MSG_DEINIT_APP, kbStrut.vkCode, 0);
    } else if (prevMode == ModeWin && (switchApp || winHoldReleasing)) {
    ...
    if (mode == ModeApp) {
      bypassMsg = true;
      if (switchApp)
        PostThreadMessage(_MainThread,
                          keyState._InvertKeyDown ? MSG_PREV_APP : MSG_NEXT_APP,
                          0, 0);
      else if (prevApp)
      ...
      bypassMsg = true;
      PostThreadMessage(_MainThread,
                        keyState._InvertKeyDown ? MSG_PREV_WIN : MSG_NEXT_WIN,
                        0, 0);
    }
  }
...
static void DrawRoundedRect(GpGraphics *pGraphics, GpPen *pPen, GpBrush *pBrush,
                            const RectF *re, float di) {
  float l = (re->X);
  float t = (re->Y);
  float b = (t + re->Height);
  float r = (l + re->Width);
...
static void Draw(SAppData *appData, HDC dc, RECT clientRect) {
  ...
  const float containerSize =
      appData->_Metrics._Container; // Height of each item
  ...
  const float digitBoxHeight =
      min(max(selectSize * 0.15f, 16.0f), selectSize * 0.5f);
  ...
  (void)digitPad; // Implicit as text centering is handled by gdip
  const float nameHeight = iconSize * 0.4f; // Text height relative to icon size
  ...
  GpFontFamily *pFontFamily = NULL;
  {
    ...
    ASSERT(Ok == GdipCreateFont(pFontFamily, nameHeight, FontStyleRegular,
                                (int)MetafileFrameUnitPixel, &fontName));
  }
  ...
  // Selection highlighting
  {
    ...
    {
      RectF selRect = {pad, pad + containerSize * mouseSelIdx,
                       appData->_Metrics._WinX - 2.0f * pad, containerSize};
      DrawRoundedRect(pGraphics, NULL, pGraphRes->_pBrushBgHighlight, &selRect,
                      10);
    }

    // Keyboard selection highlight (border)
    {
      RectF selRect = {pad, pad + containerSize * selIdx,
                       appData->_Metrics._WinX - 2.0f * pad, containerSize};
  ...
  for (uint32_t i = 0; i < appData->_WinGroups._Size; i++) {
    ...
    const float itemY = y + containerSize * i;

    // Icon on the left
    if (pWinGroup->_IconBitmap) {
      GdipDrawImageRectI(pGraphics, pWinGroup->_IconBitmap, x + padIcon,
                         itemY + padIcon, iconSize, iconSize);
    }

    // Window count badge (for apps with multiple windows)
    if (appData->_Config._AppSwitcherMode == AppSwitcherModeApp &&
        pWinGroup->_WindowCount > 1) {
      WCHAR str[] = L"\0\0";
      ...
      RectF r = {(x + padIcon + iconSize - p - w), (itemY + padIcon), (w), (h)};
      r.X = round(r.X);
      r.Y = round(r.Y);
      ...
      RectF rr = {0, 0, 0, 0};
      ASSERT(Ok == GdipAddPathString(
                       pPath, str, digitsCount, pFontFamily, FontStyleBold,
                       digitHeight * (digitsCount > 1 ? 0.8f : 1.0f), &rr,
                       pGraphRes->_pFormat));
      ...
      GpMatrix *mat;
      ASSERT(Ok == GdipCreateMatrix2(
                       1.0f, 0.0f, 0.0f, 1.0f,
                       (r.X - rr.X + (r.Width - rr.Width) * 0.5f),
                       (r.Y - rr.Y + (r.Height - rr.Height) * 0.5f), &mat));
    ...
    // Window name on the right of the icon
    {
      const float textX = x + padIcon + iconSize + pad;
      const float textWidth = appData->_Metrics._WinX - textX - pad;
      const float textY = itemY + (containerSize - nameHeight) * 0.5f;

      RectF textRect = {textX, textY, textWidth, nameHeight};

      static wchar_t name[MAX_PATH];
      int count = wcslen(pWinGroup->_AppName);
      if (count != 0) {
        // Create left-aligned string format for the window name
        GpStringFormat *pTextFormat = NULL;
        ASSERT(Ok == GdipStringFormatGetGenericDefault(&pTextFormat));
        GpStringFormat *pClonedFormat = NULL;
        ASSERT(Ok == GdipCloneStringFormat(pTextFormat, &pClonedFormat));
        ASSERT(Ok == GdipSetStringFormatAlign(
                         pClonedFormat, StringAlignmentNear)); // Left align
        ASSERT(Ok ==
               GdipSetStringFormatLineAlign(
                   pClonedFormat, StringAlignmentCenter)); // Vertical center
        ASSERT(Ok ==
               GdipSetStringFormatFlags(
                   pClonedFormat, StringFormatFlagsNoClip |
                                      StringFormatFlagsDisplayFormatControl));

        RectF rout;
        int maxCount = 0;
        GdipMeasureString(pGraphics, pWinGroup->_AppName, count, fontName,
                          &textRect, pClonedFormat, &rout, &maxCount, 0);
        wcsncpy(name, pWinGroup->_AppName, min(maxCount, count));
        if (count > maxCount) {
          wcscpy(&name[maxCount - 3], L"...");
          count = maxCount;
        }
        GdipDrawString(pGraphics, name, count, fontName, &textRect,
                       pClonedFormat, pGraphRes->_pBrushText);

        ASSERT(Ok == GdipDeleteStringFormat(pClonedFormat));
      }
    }
  }
  ...
  BitBlt(GetDC(hwnd), clientRect.left, clientRect.top,
         clientRect.right - clientRect.left, clientRect.bottom - clientRect.top,
         pGraphRes->_DC, 0, 0, SRCCOPY);
...
LRESULT CALLBACK WorkerWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
                                  LPARAM lParam) {
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
  ...
  case WM_MOUSEMOVE: {
    if (!appData->_Config._Mouse)
      return 0;
    const int itemHeight = (int)appData->_Metrics._Container;
    const int pad = (int)appData->_Metrics._Pad;
    const int posY = GET_Y_LPARAM(lParam);
    appData->_MouseSelection = min(max(0, (posY - pad) / itemHeight),
                                   (int)appData->_WinGroups._Size - 1);
  ...
  case WM_CREATE: {
    appData = (SAppData *)((CREATESTRUCTA *)lParam)->lpCreateParams;
    {
      ...
      appData->_GraphicsResources._Bitmap =
          CreateCompatibleBitmap(winDC, clientRect.right - clientRect.left,
...
#define PATCH_TILDE(key)                                                       \
  key = key == VK_OEM_3 ? MapVirtualKey(41, MAPVK_VSC_TO_VK) : key;
...
#undef PATCH_TILDE
  ...
  HANDLE threadKbHook = CreateRemoteThread(
      GetCurrentProcess(), NULL, 0, *KbHookCb, (void *)&_AppData, 0, NULL);
  (void)threadKbHook;
  ...
  while (GetMessage(&msg, NULL, 0, 0) > 0) {
    ...
    case MSG_NEXT_APP: {
      ...
      _AppData._Selection++;
      _AppData._Selection =
          Modulo(_AppData._Selection, _AppData._WinGroups._Size);
    ...
    case MSG_NEXT_WIN: {
      ...
      _AppData._Selection++;
      _AppData._Selection =
          Modulo(_AppData._Selection, _AppData._CurrentWinGroup._WindowCount);
